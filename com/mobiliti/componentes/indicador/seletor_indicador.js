...
function IndicatorSelector()
{
    ...
    this.html = function(idElement)
    {
        var button = '<div id="' + idElement + '" style="float: right;">'
                ...
                + '<button class="blue_button big_bt selector_popover" data-toggle="dropdown" style="float:right; margin-right: 29px !important; height: 34px; font-size: 14px;" rel="popover" >' + lang_mng.getString("selecionar") + '</button>'
                + '</div>'
                + '</div>';
    ...
    this.startSelector = function(multiselect, id_element_context, _listener, orientation, multiYear_, _to_hide, _skipLimit)
    {
        ...
        html += '<div><div class="dimensao box dim"><h6 class="title_box">' + lang_mng.getString("seletor_dimensao") + '</h6><ul class="nav nav-list list_menu_indicador dim">' +
                ...
                '</ul></div>' +
                '<div class="indicador box"><h6 class="title_box">' + lang_mng.getString("seletor_indicador") + '</h6><ul class="nav nav-list list_menu_indicador">' +
                '</ul></div>';
        if (multiselect == true) {
