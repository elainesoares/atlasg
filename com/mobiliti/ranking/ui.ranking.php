    ...
    if (isset($_POST["cross_data_ranking"])) {
        ...
        $ano_h = $fk_ano;
        $ranking = new Raking($ordem_id, $ordem, $pag, $espc, $start, $estado, $estados_pos, $load_more,$donwload,$fk_ano,$lang_mng);
//        $ranking->addInstance($lang_mng);
...
//        $ranking->addInstance($lang_mng);
    ...
    <div class="tabbable inlineBlock"> <!-- Only required for left/right tabs -->
        ...
        <div class="" style="float:right;margin-right: 20px;margin-top: -70px;"><span style="font-weight: bold; margin-left:0px;margin-top: 20px; width:44px;position: absolute"><?php echo $lang_mng->getString("rankin_Ano") ?></span> <?php $ranking->drawAnoSelect(); ?></div>
    </div>
