...
function set_dropdowns(){
    $.post("hdi/ano.php",function(valor){
        $("select[name='ano1']").html(valor);
        $("select[name='ano2']").html(valor);
    })
    $.post("hdi/estado.php",function(valor){
        $("select[name=estado1]").html(valor);
        $("select[name=estado2]").html(valor);
    })
    $("select[name='municipio1']").append("<option value=0>Escolha um estado</option>")
    $("select[name='municipio2']").append("<option value=0>Escolha um estado</option>")	
}
...
function set_defaults(show_outlines, show_labels){
    $("select[name='ano2']").val(0);
    $("select[name='estado2']").val(0);
    $("select[name='municipio2']").val(0);
    ...
    $('input[name="show_shadow1"]').attr("checked", true);
    $('input[name="show_shadow2"]').attr("checked", true);	
}

function dados_defaults(){
    if(!tree_options[0]){
        $.post("hdi/estadoSearch.php",{municipio:getCookie("usercityid")},function(valor){
            var x = valor.split("|");
            $.post("hdi/municipio.php",{estado:x[1]},function(valorr){
                $("select[name=municipio1]").html(valorr);
                $("select[name='estado1']").val(x[1]);
                $("select[name='municipio1']").val(x[0]);
                $("select[name='ano1']").val(2);
                $("select[name=municipio2]").html(valorr);
                $("select[name='estado2']").val(x[1]);
                $("select[name='municipio2']").val(x[0]);
                $("select[name='ano2']").val(3);
                dados();
            })
       })
    }
...
//                        table_hdi({'income': da2[7], 'health': da2[5], 'edu': da2[3], 'hdi': da2[1], 'pos': 'right'});
	...
	//if(data["mid"] && tree_options[0]){
    if(data["mid"]){
		var indicies = get_index_order(data["mid"]);
		
		if(show_shadow3){
			var shadow_indicies = get_index_order(data["mid"]);
			for(var i = 0; i < shadow_indicies.length; i++){
				shadow_indicies[i].value = 100;
				if(shadow_indicies[i].secondary_branches){
					for(var j = 0; j < shadow_indicies[i].secondary_branches.length; j++){
			...
			build_tree({"hdi":100, "indicies":shadow_indicies, "left":false, "show_outlines":show_outlines3, "show_labels":show_labels3, "opacity": 0.35, "mid":true});
		}
		
		build_tree({"hdi":data["mid"].hdi, "indicies":indicies, "left":false, "show_outlines":show_outlines3, "show_labels":show_labels3, "mid":true});
		set_table({"hdi":data["mid"], "left":false});
	} else {
            //if(tree_options[0])
		//show_no_data(false,true);
            set_table({"left":false});
	}
...
function table_hdi(option){
    ...
    if(option.pos == 'left'){
        ...
        var n = new Array();
        n[0] = data2.left.income.toString();
        n[1] = data2.left.health.toString();
        n[2] = data2.left.edu.toString();
        n[3] = data2.left.hdi.toString();
        
        for(var cont = 0; cont < n.length; cont ++){
            n[cont] = n[cont].replace('.',",");
        }
        ...
        data2.left = {
          ...
          $('.table_city1').html("<table class='city1' style='margin-bottom: 40px;'>"+
            "<tr class='tr_titulo' style='border-bottom: 1px solid #000'>"+
                "<td style='width: 253px; padding: 5px; font-weight: bold;'>IDHM e seus componentes</td>"+
                "<td style='width: 100px; padding: 5px; font-weight: bold;'>Valores</td>"+
            "</tr>"+
            "<tr class='tr_titulo' style='border-bottom: 1px solid #ccc'>"+
                "<td style='width: 100px; padding: 5px; font-weight: bold;'>IDHM</td>"+
                "<td style='width: 100px; padding: 5px;'>"+ data2.left.hdi+"</td>"+
            "</tr>"+
            "<tr class='tr_titulo' style='border-bottom: 1px solid #ccc'>"+
                "<td style='width: 100px; padding: 5px; font-weight: bold; padding-right: 85px;'>IDHM Renda</td>"+
                "<td style='width: 100px; padding: 5px;'>"+data2.left.income+"</td>"+
             "</tr>"+
             "<tr class='tr_titulo' style='border-bottom: 1px solid #ccc'>"+
                "<td style='width: 100px; padding: 5px; font-weight: bold;'>IDHM Longevidade</td>"+
                "<td style='width: 100px; padding: 5px;'>"+data2.left.health+"</td>"+
             "</tr>"+
             "<tr class='tr_titulo' style='border-bottom: 1px solid #ccc'>"+
                "<td style='width: 100px; padding: 5px; font-weight: bold;'>IDHM Educação</td>"+
    ...
    if(option.pos == 'right'){
        ...
        var n = new Array();
        n[0] = data2.right.income.toString();
        n[1] = data2.right.health.toString();
        n[2] = data2.right.edu.toString();
        n[3] = data2.right.hdi.toString();
        ...
        $('.table_city2').html("<table class='city2' style='margin-bottom: 40px; width: 356px; margin-top: -59px;'>"+
            ...
            "</tr>"+
            "<tr class='tr_titulo' style='border-bottom: 1px solid #ccc'>"+
                "<td style='width: 0px; padding: 5px; font-weight: bold; padding-right: 135px;'>IDHM Renda</td>"+
...
function get_index_order(data){
    ...
    indicies[indicies.length] = {value: data.income, color: branch_colors.income, name: "renda", secondary_branches: false}
    indicies[indicies.length] = {value: data.edu, color: branch_colors.edu, name: "edu", secondary_branches: false}
    indicies[indicies.length] = {value: data.health, color: branch_colors.health, name: "longevidade", secondary_branches: false}
    return indicies;
}
...
function make_branch(x, y, index, radius, order, opacity, show_outlines, show_labels){
	...
	sec_branch_top_txt.attr("font-weight", "bold");
            sec_branch_top_txt.attr("font-size", "11px");
...
function make_trunk(w, h, x, y, r, color, opacity, show_outlines, show_labels){
    ...
    top_txt = draw_text(top_txt_x, top_txt_y, "IDHM", -90, top_txt_x, top_txt_y);
//    top_txt.attr("font-weight", "bold");
    top_txt.attr("font-size", "12px");
...
/*function build_tree(options){
	...
	var Colors = {"trunk_edu": "#616873", "trunk_longevidade": "#79605f", "trunk_renda": "#7b735e"};
        var trunk_colors = Array();
        trunk_colors[0] = {value:options.indicies[0].value,color:Colors["trunk_"+options.indicies[0].name]};
        trunk_colors[1] = {value:options.indicies[1].value,color:Colors["trunk_"+options.indicies[1].name]};
        trunk_colors[2] = {value:options.indicies[2].value,color:Colors["trunk_"+options.indicies[2].name]};
	trunk_colors.sort(compareIndicies);
	var trunk_width = 25;
	var radius = 1;
	if(options.left){
		var trunk_position = (paper_w/5) - trunk_width;
		var base_branch_xposition = (paper_w/5) - (trunk_width / 2) + radius;
	}
	else if(options.mid) {
		var trunk_position = (paper_w*(0.5)) - trunk_width;
		var base_branch_xposition = (paper_w*(0.5)) - (trunk_width / 2) + radius;
	}
        else{
            //var trunk_position = (paper_w*(tree_options[1])) - trunk_width;
            var trunk_position = (paper_w) - trunk_width;
            //var base_branch_xposition = (paper_w*(tree_options[1])) - (trunk_width / 2) + radius;   
            var base_branch_xposition = (paper_w) - (trunk_width / 2) + radius;   
        }
	
	// Draw the trunk
	trunk_height = make_trunk(trunk_width, options.hdi, trunk_position, paper_h, radius, trunk_colors[2].color.toString(), options.opacity, options.show_outlines, options.show_labels);
...
//function show_no_data(left,mid){
function show_no_data(left){
	...
	//tirado por leo
		...
		//var trunk_position = paper_w*(2/4);
	
	var txt = paper.text(trunk_position, paper_h - 12, "Sem Dados");
	//var txt = paper.text(trunk_position, paper_h - 12, "");
	txt.attr("fill", "black");
