...
function set_defaults(show_outlines, show_labels){
	...
	if(getCookie("usercityid")){
		dados_defaults();
	}

	//$("select[name='ano1']").val(0);
	$("select[name='ano2']").val(0);
	//$("select[name='estado1']").val(0);
	$("select[name='estado2']").val(0);
	//$("select[name='municipio1']").val(0);
...
{
    alert();
				$.post("hdi/consulta2.php",{ano:2,estado:20},function(valor){
						da = valor.split("|");
						refresh();
						}
					)
...
function dados(){
	...
	var ano1 = $("select[name='ano1']").val();
	var ano2 = $("select[name='ano2']").val();
	var estado1 = $("select[name='estado1']").val();
	var estado2 = $("select[name='estado2']").val();
	var municipio1 = $("select[name='municipio1']").val();
	var municipio2 = $("select[name='municipio2']").val();
	...
	if(ano1!=0 && estado1!=0){
		if(municipio1!=0){
			if(ano1!=ano1aux || municipio1!=municipio1aux){
				...
				$.post("hdi/consulta.php",{ano:$("select[name='ano1']").val(),municipio:$("select[name='municipio1']").val()},function(valor){
					da = valor.split("|");
					refresh();
					}
				)
			}else{
				if(ano1!=ano1aux || estado1!=estado1aux){
					loading(true);
					ano1aux = ano1;
					estado1aux = estado1;
					municipio1aux = municipio1;
					
					$.post("hdi/consulta2.php",{ano:$("select[name='ano1']").val(),estado:$("select[name='estado1']").val()},function(valor){
						da = valor.split("|");
						refresh();
						}
					)
	...
	if(ano2!=0 && estado2!=0){
		if(municipio2!=0){
			if(ano2!=ano2aux || municipio2!=municipio2aux){
				...
				$.post("hdi/consulta.php",{ano:$("select[name='ano2']").val(),municipio:$("select[name='municipio2']").val()},function(valor){
					da2 = valor.split("|");
					refresh();
					}
				)
			}else{
				if(ano2!=ano2aux || estado2!=estado2aux){
					loading(false);
					ano2aux = ano2;
					estado2aux = estado2;
					municipio2aux = municipio2;
					
					$.post("hdi/consulta2.php",{ano:$("select[name='ano2']").val(),estado:$("select[name='estado2']").val()},function(valor){
						da2 = valor.split("|");
						refresh();
						}
					)
...
function make_trunk(w, h, x, y, r, color, opacity, show_outlines, show_labels){
	...
	top_txt = draw_text(top_txt_x, top_txt_y, "IDH", -90, top_txt_x, top_txt_y);
	top_txt.attr("font-weight", "bold");
...
function loading(left){
	...
	//var c = paper.image("carregando.gif", trunk_position - 25, paper_h - 50, 30, 30);
	//paper.image("img/carregando.gif", trunk_position - 25, paper_h - 70, 30, 30);
